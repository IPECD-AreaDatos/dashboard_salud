...
const query = async () => {
  const sql = "WITH caps_normalized AS ("
    ...
    + " AND LOWER(TRIM(COALESCE(s.nombre, p.nombre_establecimiento))) LIKE '%8%' "
    + " AND LOWER(TRIM(COALESCE(s.nombre, p.nombre_establecimiento))) LIKE '%teresita%' "
    ...
    + " ) "
    + " SELECT raw_name, raw_name_lower, normalized_name, COUNT(*) "
    + " FROM caps_normalized_step2 "
    + " GROUP BY raw_name, raw_name_lower, normalized_name "
    + " ORDER BY COUNT(*) DESC, raw_name;";
